...
from financeiro.trava_fechamento import exigir_aberta
from financeiro.escopo import SISTEMA, clausula, exigir_admin, exigir_escrita
from financeiro.db import get_conn
from financeiro.dinheiro import para_centavos, para_reais
from financeiro.ordenacao import ordenar_linhas
from financeiro.visao import (
    FILTRO_SQL_NOTAS,
    SEM_VALOR,
    casa_data,
    formatar_valor,
...
from financeiro.reports.contas_pagar import _separar_categoria
...
def linha_de_servico(resumo: dict, detalhe: dict, empresa: str) -> dict:
    ...
    return {
        ...
        # preenchimento — ver FILTRO_SQL_NOTAS em financeiro/visao.py, que deixa passar
...
def definir_marcacao(
    escopo, empresa: str, tipo_nota: str, id_nota: str, considerar: bool | None
) -> bool:
    ...
    try:
        ...
        auditoria.registrar(
            ...
            f"{tipo_nota}:{id_nota}",
            empresa,
            {"considerar_manual": linha["considerar_manual"]},
            {"considerar_manual": valor},
        )
        conn.commit()
        return True
    finally:
...
def listar_notas(
    ...
    validos = {"empresa", "tipo_nota", "situacao", "descricao_situacao", "numero", "cliente_nome"}
    ...
    if consideracao:
        linhas = [l for l in linhas if rotulo_consideracao(l["considerar_efetivo"]) in consideracao]
...
def sincronizar_notas(cliente, empresa_nome: str, data_ini, data_fim, progresso=None) -> dict:
    ...
    receita = sum(
        l["valor"] or 0
        for l in listar_notas(SISTEMA, {"empresa": empresa_nome})
