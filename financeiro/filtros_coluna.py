...
from financeiro.visao import SEM_VALOR, formatar_valor, rotulo_consideracao
...
COLUNAS["receitas_vendas"] = {c: v for c, v in COLUNAS["receitas"].items() if c != "tipo_nota"}
...
# filtro e ordem em Python (financeiro/consulta.py:alertas_listagem).
COLUNAS["alertas"] = {
    "situacao": None,
    "tipo_rotulo": None,
    "empresa": None,
...
def _montar_arvore(ordenados: list[str], coluna: str, tem_vazio: bool) -> list[dict]:
    ...
    for ano in sorted(anos):
        ...
        for mm in sorted(anos[ano]):
            dias = [
                {"valor": d, "rotulo": d[0:2]} for d in sorted(anos[ano][mm], key=lambda x: x[0:2])
            ]
...
def valores(
    ...
    return {"tipo": "lista", "valores": cortados, "total": total, "truncado": total > len(cortados)}
