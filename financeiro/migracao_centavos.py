...
Roda dentro da transação da migração (financeiro/db.py:migrar). A prova de
...
verificado já foi gravado antes):
  ...
  1. LINHA A LINHA: o centavo gravado pelo SQL (ROUND(valor * 100)) tem de
     ...
     (financeiro/dinheiro.para_centavos). E a conversão tem de ser reversível: o
...
from financeiro.dinheiro import para_centavos

log = logging.getLogger("financeiro.db")
