...
Por que não `os.environ` nem `load_dotenv`: em produção este sistema divide o
processo com outros sistemas, e o ambiente do processo pode ter variáveis
deles — inclusive uma `SECRET_KEY`. Ler o ambiente herdaria a chave de outro
sistema. Por isso:
...
# O .env do projeto. Variável de módulo (e não caminho fixo dentro da função)
# para os testes o trocarem por um arquivo inexistente: a suíte não pode
# depender do .env da máquina — no servidor ele tem prefixo /financeiro e
# cookie Secure, e 122 testes quebravam no ensaio do deploy.
...
def variaveis(caminho_env: Path | None = None) -> dict[str, str]:
    ...
    arquivo = caminho_env or ARQUIVO_ENV
    if arquivo.exists():
        valores.update(
            {
                k: v
...
def config_flask(sobrescrever: dict | None = None) -> dict:
    ...
    cfg = {
        ...
        "PREFIXO": prefixo,
        # "production" só no servidor (o instalar.sh grava). Fora dele, a tela
        # de Sincronizar avisa que o banco local deixou de ser a fonte da verdade.
        "AMBIENTE": v.get("GSF_AMBIENTE") or "development",
