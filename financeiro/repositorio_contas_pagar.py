...
from financeiro.escopo import clausula, exigir_admin, exigir_escrita
...
# em reais Decimal (valor...). Ver financeiro/dinheiro.py.
...
def upsert_contas(escopo, linhas: list[dict]) -> None:
    ...
    set_clause = ", ".join(f"{c}=excluded.{c}" for c in colunas_banco if c not in ("empresa", "id"))
...
def listar_contas(
    ...
    if competencias is not None:
        linhas = [l for l in linhas if casa_data(l["competencia"], competencias)]
