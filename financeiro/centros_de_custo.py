...
from financeiro.dinheiro import ZERO, arredondar, como_decimal, ratear
from financeiro.resumos import SEM_CATEGORIA
...
def _linha(nome: str, rateia: bool = True) -> dict:
    base = {"nome": nome, "receita": ZERO, "despesa": ZERO, "qtd_receita": 0, "qtd_despesa": 0}
...
def _fechar(linha: dict) -> dict:
    ...
    linha["custo_total"] = linha["despesa"] + sum(linha[campo] or 0 for campo in CAMPOS_RATEADOS)
    ...
    linha["margem"] = linha["resultado"] / linha["receita"] * 100 if linha["receita"] else None
...
def separar(notas: list[dict], contas: list[dict]) -> dict:
    ...
    def caixa(bloco: str, categoria: str | None) -> dict:
        ...
        return blocos[bloco].setdefault(nome, _linha(nome, rateia=bloco != SEM_CLASSIFICACAO))
